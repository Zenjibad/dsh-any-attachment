...
- The `@name` renders as a tagged span in the composer (lexicon decoration).
...
- **Client** (`client/`): composer `+` button (`conversation.input.left`), a capture-phase drop handler, and an `@file` input-trigger source (workspace autocomplete, pathless picks, lexicon for decoration). Rasters route through `createDraftImages`/`addImages`; everything else uploads via the channel and the mention `@<name>` is appended to the composer draft via `inputActions.setDraft`.
